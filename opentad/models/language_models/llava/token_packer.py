...
class TokenPackerAttention(nn.Module):
    ...
    def forward(self, x, x_multi, attn_mask=None):
        ...
        if self.patch_devide_pattern == 'spatial':
            q = self.divide_feature(q, 1, 1, query_temporal_num, query_spatial_num,
                                    c)  # 1, query_temporal_num*query_spatial_num, c
            k = self.divide_feature(k, 1, self.spatial_scale_factor, key_temporal_num, key_spatial_num,
                                    c)  # kernel_size, query_temporal_num*query_spatial_num, c
            v = self.divide_feature(v, 1, self.spatial_scale_factor, key_temporal_num, key_spatial_num,
                                    c)  # kernel_size, query_temporal_num*query_spatial_num, c
        elif self.patch_devide_pattern == 'temporal':
            q = self.divide_feature(q, 1, 1, query_temporal_num, query_spatial_num,
                                    c)  # 1, query_temporal_num*query_spatial_num, c
            k = self.divide_feature(k, self.temporal_scale_factor, 1, key_temporal_num, key_spatial_num,
                                    c)  # kernel_size, query_temporal_num*query_spatial_num, c
            v = self.divide_feature(v, self.temporal_scale_factor, 1, key_temporal_num, key_spatial_num,
                                    c)  # kernel_size, query_temporal_num*query_spatial_num, c
        elif self.patch_devide_pattern == 'spatial_temporal':
            q = self.divide_feature(q, 1, 1, query_temporal_num, query_spatial_num,
                                    c)  # 1, query_temporal_num*query_spatial_num, c
            k = self.divide_feature(k, self.temporal_scale_factor, self.spatial_scale_factor, key_temporal_num,
                                    key_spatial_num, c)  # kernel_size, query_temporal_num*query_spatial_num, c
            v = self.divide_feature(v, self.temporal_scale_factor, self.spatial_scale_factor, key_temporal_num,
                                    key_spatial_num, c)  # kernel_size, query_temporal_num*query_spatial_num, c
        else:
            raise ValueError("Unexpected patch devide pattern")

        self.attn.out_proj.weight=self.attn.out_proj.weight.to(dtype=torch.float16)
