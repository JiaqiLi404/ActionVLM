...
def eval_one_epoch(
    ...
    if rank == 0:
        ...
        if not not_eval and len(result_dict) > 0:
            # build evaluator
            ...
            for vid, pred_list in result_eval['results'].items():
                for pred in pred_list:
                    pred_id = f"{vid}-{pred['segment'][0]}-{pred['segment'][1]}-{class_index[pred['label']]}"
                    # tiou_all_vid=[k for k,v in tiou_all.items() if k.startswith(vid)]
                    pred['iou'] = tiou_all[pred_id] if pred_id in tiou_all else 0.0
                    pred['valid'] = pred_id in valid_prediction_ids

        if cfg.post_processing.save_dict:
            result_path = os.path.join(cfg.work_dir, f"result_detection {curr_epoch} {average_mAP}.json")
