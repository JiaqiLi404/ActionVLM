...
class Adapter(BaseModule):
    def __init__(
        ...
        self.cls_temporal_mlp = nn.Linear(hidden_dims, hidden_dims)
        trunc_normal_init(self.cls_temporal_mlp, std=math.sqrt(2.0 / hidden_dims), bias=0)
    ...
    def forward(self, x: torch.Tensor, h: int, w: int) -> torch.Tensor:
        ...
        # temporal depth-wise convolution
        B, N, C = x.shape  # 192, 8*10*10+1, 768
        t = self.temporal_size
        num_patches = N - 1
        patch_tokens = x[:, 1:, :]  # [B, t*h*w, C]
        cls_token = x[:, :1, :]  # [B, 1, C]

        patch_5d = patch_tokens.reshape(-1, t, h, w, C)
        attn = patch_5d.permute(0, 2, 3, 4, 1).flatten(0, 2)  # [B*h*w, C, t]
        attn = self.dwconv(attn)
        attn = self.conv(attn)
        attn = attn.unflatten(0, (-1, h, w)).permute(0, 4, 1, 2, 3)
        patch_out = attn.reshape(B, num_patches, C)

        cls_out = self.cls_temporal_mlp(cls_token)  # [B, 1, C]

        patch_tokens = patch_tokens + patch_out
        cls_token = cls_token + cls_out
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            use_fused_rmsnorm=False, use_adapter: bool = False, total_frames: int = None):
        super().__init__()
        ...
        if self.use_adapter:
            self.adapter = Adapter(
                ...
                kernel_size=3,
                dilation=1,
                temporal_size=total_frames,
                mlp_ratio=0.25,
            )

    def forward(self, x, residual=None, h=None, w=None):
...
class DistInternVideo2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if pos_embed.shape[1] != x.shape[1]:
            grid_old = int(((pos_embed.shape[1] - 1) // self.num_frames) ** 0.5)
            grid_new = int(((x.shape[1] - 1) // self.num_frames) ** 0.5)

            def interpolate_embed(pos_embed):
                cls_pos_embed = pos_embed[:, :1, :]
                pos_embed = pos_embed[:, 1:, :]

                # reshape to (1, C, H_old, W_old)
                pos_embed = pos_embed.reshape(-1, grid_old, grid_old, self.embed_dim)
                pos_embed = pos_embed.permute(0, 3, 1, 2)  # (1, C, H_old, W_old)

                # bicubic interpolate on spatial dimensions
                ...
                # flatten back to (1, N_new, C)
                pos_embed = pos_embed.permute(0, 2, 3, 1).reshape(1, -1, self.embed_dim)
                pos_embed = torch.cat([cls_pos_embed, pos_embed], dim=1)
                return pos_embed

            pos_embed = interpolate_embed(pos_embed)
            # clip_pos_embed = interpolate_embed(clip_pos_embed)

        x = x + pos_embed

        # mask tokens, ~mask means visible
        if mask is None:
            mask = torch.zeros(B, T * L + 1).to(dtype=torch.bool, device=x.device)
        ...
        for idx, blk in enumerate(self.blocks):
            if isinstance(x, tuple) and len(x) == 2:
                x, residual = x
            x = blk(x, residual=residual, h=int(L ** 0.5), w=int(L ** 0.5))
