...
from opentad.models.language_models.llava.llava_arch import LlavaMetaModel, LlavaMetaForCausalLM, position_transfer, \
    reparam
    ...
    def forward(
        ...
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds,
             labels) = self.prepare_inputs_labels_for_multimodal_video(input_ids, position_ids, attention_mask,
                                                                       past_key_values, labels, images, modalities,
                                                                       image_sizes, variables)
            ...
            if labels is not None and all(variables) and self.model.has_init_specific_embeddings:
                ...
                for cur_video_idx, (cur_labels, cur_variables) in enumerate(zip(labels, variables)):
                    ...
                    shift_logits = shift_logits.view(-1,
                                                     self.config.vocab_size + self.num_temporal_tokens + self.num_spatial_tokens * 2)
                    ...
                    torch.where(cur_labels == vision_config.spatial_height_output_token_id)[0]
                    spatial_width_output_token_indices = \
                    torch.where(cur_labels == vision_config.spatial_width_output_token_id)[0]
                    if len(temporal_output_token_indices) > 0 or len(spatial_height_output_token_indices) > 0 or len(
                            spatial_width_output_token_indices) > 0:
                        cur_onehot_labels = torch.zeros(*cur_labels.shape,
                                                        self.vocab_size + self.num_temporal_tokens + self.num_spatial_tokens * 2)
                        for i in range(cur_labels.shape[0]):
                            if cur_labels[i] != -100:
                                cur_onehot_labels[i, cur_labels[i]] = 1
                        for i in range(temporal_output_token_indices.shape[0]):
                            floor_position, ceil_position, ratio = position_transfer(cur_temporal_output_locations[i],
                                                                                     self.num_temporal_tokens)
                            cur_onehot_labels[[temporal_output_token_indices[i]]][
                                vision_config.temporal_output_token_id] = 0
                            cur_onehot_labels[[temporal_output_token_indices[i]]][
                                self.vocab_size + floor_position] = 1 - ratio
                            if floor_position != ceil_position:
                                cur_onehot_labels[[temporal_output_token_indices[i]]][
                                    self.vocab_size + ceil_position] = ratio

                        for i in range(spatial_height_output_token_indices.shape[0]):
                            floor_position, ceil_position, ratio = position_transfer(
                                cur_spatial_height_output_locations[i], self.num_spatial_tokens)
                            cur_onehot_labels[[spatial_height_output_token_indices[i]]][
                                vision_config.spatial_height_output_token_id] = 0
                            cur_onehot_labels[[spatial_height_output_token_indices[i]]][
                                self.vocab_size + self.num_temporal_tokens + floor_position] = 1 - ratio
                            if floor_position != ceil_position:
                                cur_onehot_labels[[spatial_height_output_token_indices[i]]][
                                    self.vocab_size + self.num_temporal_tokens + ceil_position] = ratio

                        for i in range(spatial_width_output_token_indices.shape[0]):
                            floor_position, ceil_position, ratio = position_transfer(
                                cur_spatial_width_output_locations[i], self.num_spatial_tokens)
                            cur_onehot_labels[[spatial_width_output_token_indices[i]]][
                                vision_config.spatial_width_output_token_id] = 0
                            cur_onehot_labels[[spatial_width_output_token_indices[i]]][
                                self.vocab_size + self.num_temporal_tokens + self.num_spatial_tokens + floor_position] = 1 - ratio
                            if floor_position != ceil_position:
                                cur_onehot_labels[[spatial_width_output_token_indices[i]]][
                                    self.vocab_size + self.num_temporal_tokens + self.num_spatial_tokens + ceil_position] = ratio
                        ...
                        loss = F.cross_entropy(shift_logits, shift_labels) if loss is None else loss + F.cross_entropy(
    ...
    def generate(
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds,
             _) = self.prepare_inputs_labels_for_multimodal_video(inputs, position_ids, attention_mask, None, None,
