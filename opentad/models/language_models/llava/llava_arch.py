...
from opentad.models.language_models.llava.token_packer import TokenPacker
from opentad.utils import rank0_print
from opentad.models.language_models.llava.siglip_encoder import SigLipVisionTower
...
def reparam(weight, reparam_mat):
    reparam_weight = reparam_mat.to(weight.device, dtype=torch.float32) @ weight.to(torch.float32)
    return weight + (reparam_weight - reparam_weight.detach()).to(weight.dtype)
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            resampler_type = getattr(config, "mm_resampler_type", None)
            vision_tower=getattr(config, "mm_vision_tower", getattr(config, "vision_tower", None))
            projector_type = getattr(config, "mm_projector_type", "linear")
            if not "siglip" in vision_tower:
                raise ValueError(
                    f"Unsupported vision tower {vision_tower}. Please use a SigLip vision tower."
                )
            if resampler_type != "fast_slow_resampler":
                raise ValueError(
                    f"Unsupported resampler type {resampler_type}. Please use 'fast_slow_resampler'."
                )
            mlp_gelu_match = re.match(r"^mlp(\d+)x_gelu$", projector_type)
            if not mlp_gelu_match:
                raise ValueError(
                    f"Unsupported projector type {projector_type}. Please use 'mlpNx_gelu' where N is an integer."
                )
            self.vision_tower = SigLipVisionTower(vision_tower, vision_tower_cfg=config,delay_load=delay_load)
            config.vision_tower=vision_tower
            self.vision_resampler = TokenPacker(config, vision_tower=self.vision_tower)
    ...
    def init_special_embeddings(self):
        ...
        self.spatial_height_input_embeddings = torch.nn.Embedding(self.vision_config.spatial_token_num,
                                                                  self.config.hidden_size)
        self.spatial_height_output_embeddings = torch.nn.Linear(self.config.hidden_size,
                                                                self.vision_config.spatial_token_num, bias=False)

        self.spatial_width_input_embeddings = torch.nn.Embedding(self.vision_config.spatial_token_num,
                                                                 self.config.hidden_size)
        self.spatial_width_output_embeddings = torch.nn.Linear(self.config.hidden_size,
                                                               self.vision_config.spatial_token_num, bias=False)
        ...
        self.temporal_output_embeddings = torch.nn.Linear(self.config.hidden_size, self.vision_config.fast_frame_num,
                                                          bias=False)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, split_sizes=None, modalities=None,
        ...
        for modality, image in zip(modalities, images):
            ...
            if self.model.has_init_specific_embeddings:
                ...
                resize_spatial_height_information_injection = F.interpolate(spatial_height_information_injection,
                                                                            size=(h,), mode='linear').permute(0, 2,
                                                                                                              1).unsqueeze(
                    2)
                resize_spatial_width_information_injection = F.interpolate(spatial_width_information_injection,
                                                                           size=(w,), mode='linear').permute(0, 2,
                                                                                                             1).unsqueeze(
                    1)
                temporal_information_injection=temporal_information_injection[:image_feature.shape[0],:,:]
                if modality == "video":
                    resize_temporal_information_injection = temporal_information_injection.unsqueeze(1)
                if modality == "image":
                    resize_temporal_information_injection = temporal_information_injection[0:1].unsqueeze(
                        1) + temporal_information_injection.sum() * 0.
    ...
    def prepare_inputs_labels_for_multimodal_video(
        ...
        if self.model.has_init_specific_embeddings:
            temporal_input_embeddings = reparam(self.model.temporal_input_embeddings.weight,
                                                self.model.temporal_reparam_mat)
            temporal_output_embeddings = reparam(self.model.temporal_output_embeddings.weight,
                                                 self.model.temporal_reparam_mat)
            spatial_height_input_embeddings = reparam(self.model.spatial_height_input_embeddings.weight,
                                                      self.model.spatial_height_reparam_mat)
            spatial_height_output_embeddings = reparam(self.model.spatial_height_output_embeddings.weight,
                                                       self.model.spatial_height_reparam_mat)
            spatial_width_input_embeddings = reparam(self.model.spatial_width_input_embeddings.weight,
                                                     self.model.spatial_width_reparam_mat)
            spatial_width_output_embeddings = reparam(self.model.spatial_width_output_embeddings.weight,
                                                      self.model.spatial_width_reparam_mat)

            temporal_information_injection = ((temporal_input_embeddings + temporal_output_embeddings) / 2).unsqueeze(1)
            spatial_height_information_injection = (
                    (spatial_height_input_embeddings + spatial_height_output_embeddings) / 2).unsqueeze(1)
            spatial_width_information_injection = (
                    (spatial_width_input_embeddings + spatial_width_output_embeddings) / 2).unsqueeze(1)

            device = self.get_model().embed_tokens.weight.device
            inputs_embeds = F.embedding(input_ids, torch.cat(
                [self.get_model().embed_tokens.weight, temporal_input_embeddings.to(device),
                 spatial_height_input_embeddings.to(device), spatial_width_input_embeddings.to(device)]
            ))
        else:
        ...
        if (input_ids.shape[1] != 1 or self.training):
            ...
            encoded_image_features, encoded_fast_image_features = self.encode_images(concat_images, split_sizes,
                                                                                     modalities,
                                                                                     temporal_information_injection,
                                                                                     spatial_height_information_injection,
                                                                                     spatial_width_information_injection)

            new_input_embeds = []
            for cur_video_idx, (cur_input_ids, cur_input_embeds, modality) in enumerate(
                    zip(input_ids, inputs_embeds, modalities)):
                if modality == 'image':
                    ...
                    for video_start_token_pos in video_start_tokens:
                        cur_video_features = encoded_fast_image_features[cur_video_idx].to(
                    ...
                    if self.vision_config.slow_token:
                        slow_video_start_tokens = torch.where(cur_input_ids == self.vision_config.slow_vid_start_token)[
                            0]

                        for video_start_token_pos in slow_video_start_tokens:
                            cur_video_features = encoded_image_features[cur_video_idx].to(
                                device=cur_input_embeds.device)

                            cur_video_features = cur_video_features.flatten(0, 1)
                            num_patches = cur_video_features.shape[0]
                            if cur_input_ids[
                                video_start_token_pos + num_patches - 1] != self.vision_config.slow_vid_end_token:
                                raise ValueError("The video end token should follow the video start token.")
                            if orig_embeds_params is not None:
                                cur_new_input_embeds = torch.cat((
                                    cur_input_embeds[:video_start_token_pos],
        ...
        if (input_ids.shape[1] != 1 or self.training) and all(variables):
            ...
            for cur_video_idx, (cur_input_ids, cur_input_embeds, cur_variables) in enumerate(
                    zip(input_ids, inputs_embeds, variables)):
                ...
                if (cur_input_ids == self.vision_config.temporal_input_token_id).sum() \
                    ...
                    if (cur_input_ids == self.vision_config.temporal_input_token_id).sum() > len(
                            cur_temporal_input_locations) \
                            or (cur_input_ids == self.vision_config.temporal_output_token_id).sum() > len(
                        cur_temporal_output_locations):
                        raise ValueError(
                            "The number of temporal tokens and input temporal location features should be the same.")
                    if (cur_input_ids == self.vision_config.temporal_input_token_id).sum() > len(
                            cur_temporal_input_locations) \
                            or (cur_input_ids == self.vision_config.temporal_output_token_id).sum() > len(
                        cur_temporal_output_locations) \
                            or (cur_input_ids == self.vision_config.spatial_height_input_token_id).sum() > len(
                        cur_spatial_height_input_locations) \
                            or (cur_input_ids == self.vision_config.spatial_height_output_token_id).sum() > len(
                        cur_spatial_height_output_locations) \
                            or (cur_input_ids == self.vision_config.spatial_width_input_token_id).sum() > len(
                        cur_spatial_width_input_locations) \
                            or (cur_input_ids == self.vision_config.spatial_width_output_token_id).sum() > len(
                        cur_spatial_width_output_locations):
                        raise ValueError(
                            "The number of spatial tokens and input temporal location features should be the same.")

                    temporal_input_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.temporal_input_token_id)[0]
                    temporal_output_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.temporal_output_token_id)[0]
                    for i, index in enumerate(temporal_input_token_indices):
                        cur_temporal_location_feature = token_transfer(cur_temporal_input_locations[i],
                    ...
                    for i, index in enumerate(temporal_output_token_indices):
                        cur_temporal_location_feature = token_transfer(cur_temporal_output_locations[i],
                                                                       temporal_input_embeddings)
                        cur_new_input_embeds[index] = cur_temporal_location_feature

                    spatial_height_input_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.spatial_height_input_token_id)[0]
                    spatial_height_output_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.spatial_height_output_token_id)[0]
                    for i, index in enumerate(spatial_height_input_token_indices):
                        cur_spatial_height_location_feature = token_transfer(cur_spatial_height_input_locations[i],
                    ...
                    for i, index in enumerate(spatial_height_output_token_indices):
                        cur_spatial_height_location_feature = token_transfer(cur_spatial_height_output_locations[i],
                                                                             spatial_height_input_embeddings)
                        cur_new_input_embeds[index] = cur_spatial_height_location_feature

                    spatial_width_input_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.spatial_width_input_token_id)[0]
                    spatial_width_output_token_indices = \
                        torch.where(cur_input_ids == self.vision_config.spatial_width_output_token_id)[0]
                    for i, index in enumerate(spatial_width_input_token_indices):
                        cur_spatial_width_location_feature = token_transfer(cur_spatial_width_input_locations[i],
                                                                            spatial_width_input_embeddings)
                        cur_new_input_embeds[index]
                    for i, index in enumerate(spatial_width_output_token_indices):
                        cur_spatial_width_location_feature = token_transfer(cur_spatial_width_output_locations[i],
                                                                            spatial_width_input_embeddings)
    ...
    def initialize_image_tokenizer(self, tokenizer):
        ...
        num_new_tokens = tokenizer.add_tokens([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_IMAGE_PATCH_TOKEN],
                                              special_tokens=True)
        vision_config.im_start_token, vision_config.im_end_token, vision_config.im_patch_token = tokenizer.convert_tokens_to_ids(
            [DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_IMAGE_PATCH_TOKEN])
    ...
    def initialize_video_tokenizer(self, tokenizer):
        ...
        num_new_tokens = tokenizer.add_tokens(
            [DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, DEFAULT_VIDEO_PATCH_TOKEN], special_tokens=True)
        vision_config.vid_start_token, vision_config.vid_end_token, vision_config.vid_patch_token = tokenizer.convert_tokens_to_ids(
            [DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, DEFAULT_VIDEO_PATCH_TOKEN])
        if vision_config.slow_token:
            num_new_tokens += tokenizer.add_tokens([DEFAULT_SLOW_VID_START_TOKEN, DEFAULT_SLOW_VID_END_TOKEN],
                                                   special_tokens=True)
            vision_config.slow_vid_start_token, vision_config.slow_vid_end_token = tokenizer.convert_tokens_to_ids(
                [DEFAULT_SLOW_VID_START_TOKEN, DEFAULT_SLOW_VID_END_TOKEN])
    ...
    def initialize_spatial_temporal_tokens(self, tokenizer, num_temporal_tokens, num_spatial_tokens):
        ...
        num_new_tokens = tokenizer.add_tokens(
            [TEMPORAL_INPUT_TOKEN, TEMPORAL_OUTPUT_TOKEN, SPATIAL_HEIGHT_INPUT_TOKEN, SPATIAL_HEIGHT_OUTPUT_TOKEN,
             SPATIAL_WIDTH_INPUT_TOKEN, SPATIAL_WIDTH_OUTPUT_TOKEN], special_tokens=True)
        vision_config.temporal_input_token_id = tokenizer.convert_tokens_to_ids([TEMPORAL_INPUT_TOKEN])[0]
        vision_config.temporal_output_token_id = tokenizer.convert_tokens_to_ids([TEMPORAL_OUTPUT_TOKEN])[0]
        vision_config.spatial_height_input_token_id = tokenizer.convert_tokens_to_ids([SPATIAL_HEIGHT_INPUT_TOKEN])[0]
        vision_config.spatial_height_output_token_id = tokenizer.convert_tokens_to_ids([SPATIAL_HEIGHT_OUTPUT_TOKEN])[0]
    ...
    def initialize_spatial_tokens(self, tokenizer, num_spatial_tokens):
        ...
        self.spatial_width_tokens = [SPATIAL_WIDTH_TOKEN_FORMAT.format(i) for i in range(num_spatial_tokens)]
        num_new_tokens = tokenizer.add_tokens(
            [SPATIAL_HEIGHT_INPUT_TOKEN, SPATIAL_HEIGHT_OUTPUT_TOKEN, SPATIAL_WIDTH_INPUT_TOKEN,
             SPATIAL_WIDTH_OUTPUT_TOKEN], special_tokens=True)
