...
def post_processing(noun_pred, verb_pred, metas, post_cfg):
    ...
    for i in range(len(metas)):
        ...
        pred_idx = (
                pred_idx
                + torch.arange(0, pred_idx.size(0), device=pred_idx.device)[:,
                  None] * num_classes_noun * num_classes_verb
        )
...
def eval_one_epoch(
    ...
    for data_dict_noun, data_dict_verb in tqdm.tqdm(zip(test_loader_noun, test_loader_verb), disable=(rank != 0)):
        # inference noun
        with torch.amp.autocast('cuda', dtype=torch.float16, enabled=use_amp_noun):
            with torch.no_grad():
                results_noun = model_noun(**data_dict_noun, infer_cfg=cfg_noun.inference)

        # inference verb
        with torch.amp.autocast('cuda', dtype=torch.float16, enabled=use_amp_verb):
            with torch.no_grad():
