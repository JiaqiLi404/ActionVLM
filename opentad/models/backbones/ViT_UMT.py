...
class VisionTransformerUMT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        B, P, C = x.size()
        grid = int((P // self.num_frames) ** 0.5)
        T = self.num_frames

        if self.pos_embed is not None:
            if P != self.pos_embed.shape[1]:
                ori_grid = int((self.pos_embed.shape[1] // self.num_frames) ** 0.5)
                pos = self.pos_embed.reshape(T, ori_grid, ori_grid, C).permute(0, 3, 1, 2)  # → [T,C, Gh, Gw]

                # bicubic resize
                pos = F.interpolate(
                    pos, size=(grid, grid), mode="bicubic", align_corners=False
                )

                pos = pos.permute(0, 2, 3, 1).reshape(1, T * grid * grid, C)
            else:
                pos = self.pos_embed
            # x = x + self.pos_embed.expand(B, -1, -1).type_as(x).to(x.device).clone().detach()
            x = x + pos.expand(B, -1, -1).type_as(x).to(x.device)
        x = self.pos_drop(x)
        ...
        if self.return_feat_map:
            x = x.reshape(B, -1, grid, grid, self.embed_dim)
