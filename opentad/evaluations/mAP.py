...
class mAP:
    ...
    def multi_thread_compute_average_precision(self):
        ...
        valid_prediction_ids = set()
        for i, cidx in enumerate(self.activity_index.values()):
            ap[:, cidx] = self.mAP_result_dict[i][0]
            tiou_all.update(self.mAP_result_dict[i][1])
            ap_per_label[cidx] = self.mAP_result_dict[i][2]
            valid_prediction_ids.update(self.mAP_result_dict[i][3])
        return ap, tiou_all, ap_per_label, valid_prediction_ids
    ...
    def evaluate(self):
        ...
        method.
        """
        self.ap, self.tiou_all, self.ap_per_label, self.valid_prediction_ids = self.multi_thread_compute_average_precision()
        ...
        metric_dict["tiou_all"] = self.tiou_all
        metric_dict["average_mAP"] = self.average_mAP
        metric_dict["ap_per_label"] = self.ap_per_label
        metric_dict["valid_prediction_ids"] = self.valid_prediction_ids
    ...
    def logging(self, logger=None):
        ...
        pprint("Average-mAP per label:")
        str = ""
        for activity, cidx in self.activity_index.items():
            str += "{:s}:{:.2f}".format(activity, self.ap_per_label[cidx] * 100) + "%, "
        pprint(str)
...
def compute_average_precision_detection(ground_truth, prediction, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    for idx, this_pred in prediction.iterrows():
        id = f"{this_pred["video-id"]}-{round(this_pred["t-start"], 2)}-{round(this_pred["t-end"], 2)}-{this_pred["label"]}"
        try:
        ...
        # tiou_dict[id] = max(tiou_arr)
        for tidx, tiou_thr in enumerate(tiou_thresholds):
            for jdx in tiou_sorted_idx:
                tiou_dict[id] = tiou_arr[jdx]
                if tiou_arr[jdx] < tiou_thr:
                    fp[tidx, idx] = 1
                    valid_prediction_ids.add(id) if tiou_thr == 0.5 else None
                    break
    ...
    map = ap.mean()
    return ap, tiou_dict, map, valid_prediction_ids
...
def compute_topkx_recall_detection(
    ...
    for videoid, _ in ground_truth_gbvn.groups.items():
        ...
        this_pred_interval = this_pred[["t-start", "t-end"]].values[top_kx_idx]
        tiou_arr = k_segment_iou(
            this_pred_interval, this_gt[["t-start", "t-end"]].values
        )
