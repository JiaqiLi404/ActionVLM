...
def main():
    ...
    if use_amp:
        ...
        scaler = None
    dtype = getattr(cfg.solver, "dtype", "float32")
    if use_amp and dtype == "float32":
        dtype = "float16"
    ...
    optimizer, _ = build_optimizer(cfg.optimizer, model, logger)
    ...
    accumulative_step = cfg.solver.get("accumulation_steps", 1)
    ...
    try:
        for epoch in range(resume_epoch + 1, max_epoch):
            ...
            train_one_epoch(
                ...
                scaler=scaler,
                accumulative_step=accumulative_step,
                dtype=dtype,
            )
